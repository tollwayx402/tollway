...
```bash
pip install "octroi[fastapi]" uvicorn
...
```bash
cd ../express-base
OCT_AGENT_KEY=0x… node agent.js http://localhost:8000/v1/report
```
...
`@octroi/coinbase` adapter's rules, including the one that matters for
...
tw = Octroi(..., sinks=[IngestClient(api_key=os.environ["OCT_KEY"]).sink])
```
